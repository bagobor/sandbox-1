...
void Voxelize(const Mesh& mesh, uint32_t width, uint32_t height, uint32_t depth, uint32_t* volume, Vec3 minExtents, Vec3 maxExtents)
{
	...
	for (uint32_t x=0; x < width; ++x)
	{
		for (uint32_t y=0; y < height; ++y)
		{
			...
			Vec3 rayStart = minExtents + Vec3(x*delta.x + offset.x, y*delta.y + offset.y, -0.1f*extents.z); // z-coord starts somewhat outside bounds 
			...
			for (;;)
			{
				...
				//if (tree.TraceRaySlow(Point3(rayStart), rayDir, t, u, v, w, s, tri))
				{
					...
					//if (fabsf(n.z) > 0.01f)
					{
						...
						const float zhit = uint32_t((zpos-minExtents.z)/delta.z);
